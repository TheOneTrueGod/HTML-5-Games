var ServerNetworkEvents = {
	...
	_onPlayerEntity: function (data, clientId) {
		if (!ige.server.players[clientId]) {
			/*var newChar = new Character(new PlayerDef())
				.addComponent(PlayerComponent)
				.streamMode(1)
				.mount(ige.server.foregroundScene);*/
			var spawnPos = ige.worldSettings.getPlayerSpawnPosition(clientId);

			var newChar = ige.characterFactory
	      .createCharacter(new PlayerDef(), PlayerComponent);
			newChar.setPosition(spawnPos[0], spawnPos[1], 0);
			ige.server.players[clientId] = newChar;
